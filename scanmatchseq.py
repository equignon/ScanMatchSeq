...
from Bio import (
    SeqIO,
)  # For arm Mac user: might need to install Biopython from source as pip seems to install x64 version
...
def main(argv):
    ...
    parser.add_argument(
        "value_searched", help="Value searched in the two datasets", type=float
    )
    parser.add_argument("--window", help="Window size", type=int, default=4)
    parser.add_argument(
        "--threshold", help="Threshold for the sliding window", type=int, default=2
    )
    parser.add_argument(
        "--test_type",
        help="Type of test to realize ('equal', 'superior', 'inferior', 'superior or equal', 'inferior or equal')",
        type=str,
        default="equal",
    )
    ...
    df_results = pd.DataFrame(
        columns=["Position A", "Sequence A", "Sequence B", "Position B"]
    )
    ...
    for i, row in df_results.iterrows():
        ...
        end_A = int(pos_A[1])
        new_row = pd.DataFrame(
            [{"Sequence": "SeqA", "Sequence_A": start_A, "Sequence_B": end_A}]
        )
        ...
        end_B = int(pos_B[1])
        new_row = pd.DataFrame(
            [{"Sequence": "SeqB", "Sequence_A": start_B, "Sequence_B": end_B}]
        )
...
def add_dataset(df, file, value):
    for seq_record in SeqIO.parse(file, "fasta"):
        df = pd.DataFrame(
            index=np.arange(1, len(seq_record.seq) + 1),
            columns=["Nucleotide", "Reactivity"],
        )
        x = 1
